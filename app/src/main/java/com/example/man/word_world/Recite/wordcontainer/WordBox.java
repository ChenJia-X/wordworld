package com.example.man.word_world.Recite.wordcontainer;
...
public class WordBox {
    ...
    public int getWordCountByGrasp(int grasp ,int learned){       //获得数据库中某个掌握程度的单词的个数
        Cursor cursor=database.query(tableName, new String[]{"word"}, "grasp=? and learned=?", new String[]{grasp+"",learned+""}, null, null, null);
    ...
    public WordInfo getWordByGraspByRandom(int fromGrasp,int toGrasp,int learned){
        ...
        Cursor cursor=database.query(tableName, new String[]{"word","interpret","right","wrong","grasp"},"grasp=? and learned=?" , new String[]{graspInt+"",learned+""}, null, null, null);
        cursor.move(index);
    ...
    public WordInfo getWordByRandom(){
        int count=0;
        Cursor cursor=database.query(tableName, new String[]{"word","interpret","right","wrong","grasp"},"word like?" ,
